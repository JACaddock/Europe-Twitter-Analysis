...
def startSorter(dir, start, end, do_print = True, format_link = "format", error_print = False, line_limit = -1):
    ...
    for path in os.listdir(absdir):
        ...
        if(zipfile.is_zipfile(absdir +"/"+ path)):
            ...
            if file_date >= start and file_date <= end:
                ...
                sortJson("data/geoEurope/"+json_path, "data/"+json_path, format_link, True, error_print, line_limit)
...
def sortJson(inp_link, out_link, format_link, do_profiles, error_print, number_lines):
...
def userLocationSetter():
    ...
    for path in os.listdir("output/profiles"):
        if os.path.isfile("output/profiles/" + path):
            if os.path.getsize("output/profiles/" + path) > 0:
                ...
                if (current_place != {}):
                    addProfile({ "user": data["user"], "place": current_place, "mentions": data["mentions"], "tweets" : tweets }, path, current_place["country_code"])
...
if __name__ == '__main__':
    ...
    startSorter(path, 2019120416, 2020120416, error_print=True)
